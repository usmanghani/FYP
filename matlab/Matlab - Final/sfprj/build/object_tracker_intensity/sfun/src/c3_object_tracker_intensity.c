...
static SFc3_object_tracker_intensityInstanceStruct chartInstance;

/* Function Declarations */
static void initialize_c3_object_tracker_intensity(void);
static void initialize_params_c3_object_tracker_intensity(void);
static void enable_c3_object_tracker_intensity(void);
static void disable_c3_object_tracker_intensity(void);
static void finalize_c3_object_tracker_intensity(void);
static void sf_c3_object_tracker_intensity(void);
static uint16_T *c3_CentroidCol(void);
static uint16_T *c3_StartCoords(void);
static real_T *c3_DirectionCode(void);
static int32_T *c3_BlobCount(void);
static void init_dsm_address_info(void);
static void sf_save_state_c3_object_tracker_intensity(FILE *c3_file);
static void sf_load_state_c3_object_tracker_intensity(FILE *c3_file);

/* Function Definitions */
static void initialize_c3_object_tracker_intensity(void)
{
  _sfTime_ = (real_T)ssGetT(chartInstance.S);
  chartInstance.c3_is_active_c3_object_tracker_intensity = 0U;
}
...
static void sf_c3_object_tracker_intensity(void)
{
  ...
  uint16_T c3_b_CentroidCol;
  int32_T c3_em_i1;
  uint16_T c3_b_StartCoords[2];
  int32_T c3_b_BlobCount;
  uint16_T c3_ScreenCenter;
  ...
  uint16_T c3_em_u5;
  _sfTime_ = (real_T)ssGetT(chartInstance.S);
  _SFD_DATA_RANGE_CHECK((real_T)*c3_CentroidCol(), 3U);
  for(c3_em_i0 = 0; c3_em_i0 < 2; c3_em_i0 = c3_em_i0 + 1) {
    _SFD_DATA_RANGE_CHECK((real_T)c3_StartCoords()[c3_em_i0], 2U);
  }
  ...
  sf_debug_symbol_scope_add_symbol("ScreenCenter", 5, 0U, 0U, 0U, 0U, 1.0, 0,
  ...
  if(CV_EML_IF(0, 0, (real_T)c3_b_BlobCount == 0.0)) {
    ...
    c3_em_u0 = (uint32_T)c3_ScreenCenter;
    c3_em_u1 = c3_em_u0 - 20U;
    if(c3_em_u1 > c3_em_u0) {
      c3_em_u1 = 0U;
    }
    c3_em_u2 = c3_em_u1;
    if(c3_em_u2 > 65535U) {
      c3_em_u3 = MAX_uint16_T;
    } else {
      c3_em_u3 = (uint16_T)c3_em_u2;
    }
    if(CV_EML_IF(0, 1, c3_b_CentroidCol < c3_em_u3)) {
      ...
      c3_em_u4 = (uint32_T)c3_ScreenCenter + 20U;
      if(c3_em_u4 > 65535U) {
        c3_em_u5 = MAX_uint16_T;
      } else {
        c3_em_u5 = (uint16_T)c3_em_u4;
      }
      if(CV_EML_IF(0, 2, c3_b_CentroidCol > c3_em_u5)) {
...
static uint16_T *c3_StartCoords(void)
{
  return (uint16_T *)ssGetInputPortSignal(chartInstance.S, 1);
}
...
static void sf_save_state_c3_object_tracker_intensity(FILE *c3_file)
{
  fwrite(&chartInstance.c3_is_active_c3_object_tracker_intensity, 1,
   sizeof(chartInstance.c3_is_active_c3_object_tracker_intensity),
   c3_file);
}

static void sf_load_state_c3_object_tracker_intensity(FILE *c3_file)
{
  fread(&chartInstance.c3_is_active_c3_object_tracker_intensity, 1,
   sizeof(chartInstance.c3_is_active_c3_object_tracker_intensity),
   c3_file);
}

/* SFunction Glue Code */
void sf_c3_object_tracker_intensity_get_check_sum(mxArray *plhs[])
{
  ((real_T *)mxGetPr((plhs[0])))[0] = (real_T)(2250751112U);
  ((real_T *)mxGetPr((plhs[0])))[1] = (real_T)(2150684054U);
  ((real_T *)mxGetPr((plhs[0])))[2] = (real_T)(2606372925U);
  ((real_T *)mxGetPr((plhs[0])))[3] = (real_T)(3506773150U);
}

mxArray *sf_c3_object_tracker_intensity_get_autoinheritance_info(void)
{
    ...
    if(!sim_mode_is_rtw_gen(S)) {
      {
        ...
        if(chartAlreadyPresent==0) {
          ...
          _SFD_SET_DATA_PROPS(3,1,1,0,SF_UINT16,0,NULL,0,0,0,0.0,1.0,0,"CentroidCol",0);
          {
            unsigned int dimVector[1];
            dimVector[0]= 2;
            _SFD_SET_DATA_PROPS(2,1,1,0,SF_UINT16,1,&(dimVector[0]),0,0,0,0.0,1.0,0,"StartCoords",0);
          }
        ...
        _SFD_SET_DATA_VALUE_PTR(3U, c3_CentroidCol());
        _SFD_SET_DATA_VALUE_PTR(2U, c3_StartCoords());
...
static void sf_opaque_initialize_c3_object_tracker_intensity(void
 *chartInstanceVar)
{
  chart_debug_initialization(chartInstance.S);
  initialize_params_c3_object_tracker_intensity();
  initialize_c3_object_tracker_intensity();
}

static void sf_opaque_enable_c3_object_tracker_intensity(void *chartInstanceVar)
{
  enable_c3_object_tracker_intensity();
}

static void sf_opaque_disable_c3_object_tracker_intensity(void *chartInstanceVar)
{
  disable_c3_object_tracker_intensity();
}

static void sf_opaque_gateway_c3_object_tracker_intensity(void *chartInstanceVar)
{
  sf_c3_object_tracker_intensity();
}

static void sf_opaque_terminate_c3_object_tracker_intensity(void
 *chartInstanceVar)
{
  finalize_c3_object_tracker_intensity();
}

static void mdlProcessParameters_c3_object_tracker_intensity(SimStruct *S)
{
  int i;
...
static void mdlSetWorkWidths_c3_object_tracker_intensity(SimStruct *S)
{
  if(sim_mode_is_rtw_gen(S)) {
    int_T chartIsInlinable =
      (int_T)sf_is_chart_inlinable("object_tracker_intensity",3);
    ssSetStateflowIsInlinable(S,chartIsInlinable);
    ssSetEnableFcnIsTrivial(S,1);
    ssSetDisableFcnIsTrivial(S,1);
    ssSetNotMultipleInlinable(S,sf_rtw_info_uint_prop("object_tracker_intensity",3,"gatewayCannotBeInlinedMultipleTimes"));
    if(chartIsInlinable) {
      ssSetInputPortOptimOpts(S, 0, SS_REUSABLE_AND_LOCAL);
      ssSetInputPortOptimOpts(S, 1, SS_REUSABLE_AND_LOCAL);
      ssSetInputPortOptimOpts(S, 2, SS_REUSABLE_AND_LOCAL);
      sf_mark_chart_expressionable_inputs(S,"object_tracker_intensity",3,3);
      sf_mark_chart_reusable_outputs(S,"object_tracker_intensity",3,1);
    }
    if (!sf_is_chart_instance_optimized_out("object_tracker_intensity",3)) {
      int dtId;
      char *chartInstanceTypedefName =
        sf_chart_instance_typedef_name("object_tracker_intensity",3);
  ...
  ssSetChecksum0(S,(2929634407U));
  ssSetChecksum1(S,(1171801321U));
  ssSetChecksum2(S,(3663813034U));
  ssSetChecksum3(S,(3855928905U));

  ssSetExplicitFCSSCtrl(S,1);
}

static void mdlRTW_c3_object_tracker_intensity(SimStruct *S)
{
  if(sim_mode_is_rtw_gen(S)) {
    sf_write_symbol_mapping(S, "object_tracker_intensity", 3);
    ssWriteRTWStrParam(S, "StateflowChartType", "Embedded MATLAB");
  }
}

static void
sf_save_debug_c3_object_tracker_intensity(SFc3_object_tracker_intensityInstanceStruct*
...
static void
sf_load_debug_c3_object_tracker_intensity(SFc3_object_tracker_intensityInstanceStruct*
 chartInstance,FILE* file)
{
  int machineNumber = _object_tracker_intensityMachineNumber_;
  int chartNumber = chartInstance->chartNumber;
...
static void sf_c3_object_tracker_intensity_sim_ctx_io(SimStruct* S, const char
 io, FILE* file)
{
  if(io == 'r') {
    sf_load_state_c3_object_tracker_intensity(file);
    sf_load_debug_c3_object_tracker_intensity(&chartInstance, file);
  } else {
    sf_save_state_c3_object_tracker_intensity(file);
    sf_save_debug_c3_object_tracker_intensity(&chartInstance, file);
  }
}
static void mdlStart_c3_object_tracker_intensity(SimStruct *S)
{
  chartInstance.chartInfo.chartInstance = NULL;
  chartInstance.chartInfo.isEMLChart = 1;
  chartInstance.chartInfo.chartInitialized = 0;
  chartInstance.chartInfo.sFunctionGateway =
    sf_opaque_gateway_c3_object_tracker_intensity;
  chartInstance.chartInfo.initializeChart =
    sf_opaque_initialize_c3_object_tracker_intensity;
  chartInstance.chartInfo.terminateChart =
    sf_opaque_terminate_c3_object_tracker_intensity;
  chartInstance.chartInfo.enableChart =
    sf_opaque_enable_c3_object_tracker_intensity;
  chartInstance.chartInfo.disableChart =
    sf_opaque_disable_c3_object_tracker_intensity;
  chartInstance.chartInfo.mdlRTW = mdlRTW_c3_object_tracker_intensity;
  chartInstance.chartInfo.mdlStart = mdlStart_c3_object_tracker_intensity;
  chartInstance.chartInfo.mdlSetWorkWidths =
    mdlSetWorkWidths_c3_object_tracker_intensity;
  chartInstance.chartInfo.restoreLastMajorStepConfiguration = NULL;
  chartInstance.chartInfo.restoreBeforeLastMajorStepConfiguration = NULL;
  chartInstance.chartInfo.storeCurrentConfiguration = NULL;
  chartInstance.S = S;
  ssSetUserData(S,(void *)(&(chartInstance.chartInfo))); /* register the chart instance with simstruct */
  ssSetmdlSimulationContextIO(S, sf_c3_object_tracker_intensity_sim_ctx_io);
  if(!sim_mode_is_rtw_gen(S)) {
    init_dsm_address_info();
  }
}

void c3_object_tracker_intensity_method_dispatcher(SimStruct *S, int_T method,
 void *data)
{
  switch (method) {
   case SS_CALL_MDL_START:
    mdlStart_c3_object_tracker_intensity(S);
    break;
   case SS_CALL_MDL_SET_WORK_WIDTHS:
    mdlSetWorkWidths_c3_object_tracker_intensity(S);
    break;
   case SS_CALL_MDL_PROCESS_PARAMETERS:
    mdlProcessParameters_c3_object_tracker_intensity(S);
    break;
   default:
    /* Unhandled method */
    sf_mex_error_message("Stateflow Internal Error:\n"
     "Error calling c3_object_tracker_intensity_method_dispatcher.\n"
