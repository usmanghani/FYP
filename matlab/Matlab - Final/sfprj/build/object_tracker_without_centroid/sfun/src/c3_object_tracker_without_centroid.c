...
#include "c3_object_tracker_without_centroid.h"
...
static SFc3_object_tracker_without_centroidInstanceStruct chartInstance;

/* Function Declarations */
static void initialize_c3_object_tracker_without_centroid(void);
static void initialize_params_c3_object_tracker_without_centroid(void);
static void enable_c3_object_tracker_without_centroid(void);
static void disable_c3_object_tracker_without_centroid(void);
static void finalize_c3_object_tracker_without_centroid(void);
...
static void sf_save_state_c3_object_tracker_without_centroid(FILE *c3_file);
static void sf_load_state_c3_object_tracker_without_centroid(FILE *c3_file);

/* Function Definitions */
static void initialize_c3_object_tracker_without_centroid(void)
{
  _sfTime_ = (real_T)ssGetT(chartInstance.S);
  chartInstance.c3_is_active_c3_object_tracker_without_centroid = 0U;
}
...
static void sf_save_state_c3_object_tracker_without_centroid(FILE *c3_file)
{
  fwrite(&chartInstance.c3_is_active_c3_object_tracker_without_centroid, 1,
   sizeof(chartInstance.
    c3_is_active_c3_object_tracker_without_centroid), c3_file);
}

static void sf_load_state_c3_object_tracker_without_centroid(FILE *c3_file)
{
  fread(&chartInstance.c3_is_active_c3_object_tracker_without_centroid, 1,
   sizeof(chartInstance.
    c3_is_active_c3_object_tracker_without_centroid), c3_file);
}

/* SFunction Glue Code */
void sf_c3_object_tracker_without_centroid_get_check_sum(mxArray *plhs[])
{
...
mxArray *sf_c3_object_tracker_without_centroid_get_autoinheritance_info(void)
{
...
static void sf_opaque_initialize_c3_object_tracker_without_centroid(void
 *chartInstanceVar)
{
  chart_debug_initialization(chartInstance.S);
  initialize_params_c3_object_tracker_without_centroid();
  initialize_c3_object_tracker_without_centroid();
}

static void sf_opaque_enable_c3_object_tracker_without_centroid(void
 *chartInstanceVar)
{
  enable_c3_object_tracker_without_centroid();
}

static void sf_opaque_disable_c3_object_tracker_without_centroid(void
 *chartInstanceVar)
{
  disable_c3_object_tracker_without_centroid();
}

static void sf_opaque_gateway_c3_object_tracker_without_centroid(void
 *chartInstanceVar)
{
  sf_c3_object_tracker_without_centroid();
}

static void sf_opaque_terminate_c3_object_tracker_without_centroid(void
 *chartInstanceVar)
{
  finalize_c3_object_tracker_without_centroid();
}

static void mdlProcessParameters_c3_object_tracker_without_centroid(SimStruct *S)
{
  int i;
...
static void mdlSetWorkWidths_c3_object_tracker_without_centroid(SimStruct *S)
{
  if(sim_mode_is_rtw_gen(S)) {
    int_T chartIsInlinable =
      (int_T)sf_is_chart_inlinable("object_tracker_without_centroid",3);
    ssSetStateflowIsInlinable(S,chartIsInlinable);
    ssSetEnableFcnIsTrivial(S,1);
    ssSetDisableFcnIsTrivial(S,1);
    ssSetNotMultipleInlinable(S,sf_rtw_info_uint_prop("object_tracker_without_centroid",3,"gatewayCannotBeInlinedMultipleTimes"));
    if(chartIsInlinable) {
      ssSetInputPortOptimOpts(S, 0, SS_REUSABLE_AND_LOCAL);
      ssSetInputPortOptimOpts(S, 1, SS_REUSABLE_AND_LOCAL);
      ssSetInputPortOptimOpts(S, 2, SS_REUSABLE_AND_LOCAL);
      sf_mark_chart_expressionable_inputs(S,"object_tracker_without_centroid",3,3);
      sf_mark_chart_reusable_outputs(S,"object_tracker_without_centroid",3,1);
    }
    if
    (!sf_is_chart_instance_optimized_out("object_tracker_without_centroid",3)) {
      int dtId;
      char *chartInstanceTypedefName =
        sf_chart_instance_typedef_name("object_tracker_without_centroid",3);
...
static void mdlRTW_c3_object_tracker_without_centroid(SimStruct *S)
{
  if(sim_mode_is_rtw_gen(S)) {
    sf_write_symbol_mapping(S, "object_tracker_without_centroid", 3);
    ssWriteRTWStrParam(S, "StateflowChartType", "Embedded MATLAB");
  }
}

static void
sf_save_debug_c3_object_tracker_without_centroid(SFc3_object_tracker_without_centroidInstanceStruct*
...
static void
sf_load_debug_c3_object_tracker_without_centroid(SFc3_object_tracker_without_centroidInstanceStruct*
 chartInstance,FILE* file)
{
  int machineNumber = _object_tracker_without_centroidMachineNumber_;
  int chartNumber = chartInstance->chartNumber;
...
static void sf_c3_object_tracker_without_centroid_sim_ctx_io(SimStruct* S, const
 char io, FILE* file)
{
  if(io == 'r') {
    sf_load_state_c3_object_tracker_without_centroid(file);
    sf_load_debug_c3_object_tracker_without_centroid(&chartInstance, file);
  } else {
    sf_save_state_c3_object_tracker_without_centroid(file);
    sf_save_debug_c3_object_tracker_without_centroid(&chartInstance, file);
  }
}
static void mdlStart_c3_object_tracker_without_centroid(SimStruct *S)
{
  chartInstance.chartInfo.chartInstance = NULL;
  chartInstance.chartInfo.isEMLChart = 1;
  chartInstance.chartInfo.chartInitialized = 0;
  chartInstance.chartInfo.sFunctionGateway =
    sf_opaque_gateway_c3_object_tracker_without_centroid;
  chartInstance.chartInfo.initializeChart =
    sf_opaque_initialize_c3_object_tracker_without_centroid;
  chartInstance.chartInfo.terminateChart =
    sf_opaque_terminate_c3_object_tracker_without_centroid;
  chartInstance.chartInfo.enableChart =
    sf_opaque_enable_c3_object_tracker_without_centroid;
  chartInstance.chartInfo.disableChart =
    sf_opaque_disable_c3_object_tracker_without_centroid;
  chartInstance.chartInfo.mdlRTW = mdlRTW_c3_object_tracker_without_centroid;
  chartInstance.chartInfo.mdlStart = mdlStart_c3_object_tracker_without_centroid;
  chartInstance.chartInfo.mdlSetWorkWidths =
    mdlSetWorkWidths_c3_object_tracker_without_centroid;
  ...
  ssSetmdlSimulationContextIO(S,
   sf_c3_object_tracker_without_centroid_sim_ctx_io);
  if(!sim_mode_is_rtw_gen(S)) {
    init_dsm_address_info();
  }
}

void c3_object_tracker_without_centroid_method_dispatcher(SimStruct *S, int_T
 method, void *data)
{
  switch (method) {
   case SS_CALL_MDL_START:
    mdlStart_c3_object_tracker_without_centroid(S);
    break;
   case SS_CALL_MDL_SET_WORK_WIDTHS:
    mdlSetWorkWidths_c3_object_tracker_without_centroid(S);
    break;
   case SS_CALL_MDL_PROCESS_PARAMETERS:
    mdlProcessParameters_c3_object_tracker_without_centroid(S);
    break;
   default:
    /* Unhandled method */
    sf_mex_error_message("Stateflow Internal Error:\n"
     "Error calling c3_object_tracker_without_centroid_method_dispatcher.\n"
