...
unsigned int sf_process_check_sum_call( int nlhs, mxArray * plhs[], int nrhs,
 const mxArray * prhs[] )
{
  extern unsigned int sf_object_tracker_intensity_process_check_sum_call( int
   nlhs, mxArray * plhs[], int nrhs, const mxArray * prhs[] );

  if(sf_object_tracker_intensity_process_check_sum_call(nlhs,plhs,nrhs,prhs))
  return 1;
  return 0;
}
...
{
  extern unsigned int sf_object_tracker_intensity_autoinheritance_info( int
   nlhs, mxArray * plhs[], int nrhs, const mxArray * prhs[] );
  if(sf_object_tracker_intensity_autoinheritance_info(nlhs,plhs,nrhs,prhs))
  return 1;
  return 0;
}
...
extern unsigned int sf_object_tracker_intensity_method_dispatcher(SimStruct *S,
...
{
  if(sf_object_tracker_intensity_method_dispatcher(simstructPtr,chartName,method,data))
  return 1;
  return 0;
}
extern void object_tracker_intensity_terminator(void);
void sf_machine_global_terminator(void)
{
  if(sfMachineGlobalTerminatorCallable) {
    ...
    object_tracker_intensity_terminator();
    sf_debug_terminate();
  }
  return;
}
extern void object_tracker_intensity_initializer(void);
extern void object_tracker_intensity_register_exported_symbols(SimStruct* S);
extern void object_tracker_intensity_debug_initialize(void);
void sf_register_machine_exported_symbols(SimStruct* S)
{
  object_tracker_intensity_register_exported_symbols(S);
}
void sf_machine_global_initializer(SimStruct* S)
{
  ...
  if(sfMachineGlobalInitializerCallable) {
    ...
    if(!simModeIsRTWGen) {
      object_tracker_intensity_debug_initialize();
    }
    object_tracker_intensity_initializer();
  }
  return;
}
