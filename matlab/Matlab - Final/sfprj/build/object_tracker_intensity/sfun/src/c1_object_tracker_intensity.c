...
static SFc1_object_tracker_intensityInstanceStruct chartInstance;

/* Function Declarations */
static void initialize_c1_object_tracker_intensity(void);
static void initialize_params_c1_object_tracker_intensity(void);
static void enable_c1_object_tracker_intensity(void);
static void disable_c1_object_tracker_intensity(void);
static void finalize_c1_object_tracker_intensity(void);
...
static void sf_save_state_c1_object_tracker_intensity(FILE *c1_file);
static void sf_load_state_c1_object_tracker_intensity(FILE *c1_file);
...
static void initialize_c1_object_tracker_intensity(void)
{
  ...
  chartInstance.c1_is_active_c1_object_tracker_intensity = 0U;
}
...
static void sf_save_state_c1_object_tracker_intensity(FILE *c1_file)
{
  fwrite(&chartInstance.c1_is_active_c1_object_tracker_intensity, 1,
   sizeof(chartInstance.c1_is_active_c1_object_tracker_intensity),
   c1_file);
...
static void sf_load_state_c1_object_tracker_intensity(FILE *c1_file)
{
  fread(&chartInstance.c1_is_active_c1_object_tracker_intensity, 1,
   sizeof(chartInstance.c1_is_active_c1_object_tracker_intensity),
   c1_file);
...
/* SFunction Glue Code */
void sf_c1_object_tracker_intensity_get_check_sum(mxArray *plhs[])
{
...
mxArray *sf_c1_object_tracker_intensity_get_autoinheritance_info(void)
{
...
static void sf_opaque_initialize_c1_object_tracker_intensity(void
 *chartInstanceVar)
{
  chart_debug_initialization(chartInstance.S);
  initialize_params_c1_object_tracker_intensity();
  initialize_c1_object_tracker_intensity();
}

static void sf_opaque_enable_c1_object_tracker_intensity(void *chartInstanceVar)
{
  enable_c1_object_tracker_intensity();
}

static void sf_opaque_disable_c1_object_tracker_intensity(void *chartInstanceVar)
{
  disable_c1_object_tracker_intensity();
}

static void sf_opaque_gateway_c1_object_tracker_intensity(void *chartInstanceVar)
{
  sf_c1_object_tracker_intensity();
}

static void sf_opaque_terminate_c1_object_tracker_intensity(void
 *chartInstanceVar)
{
  finalize_c1_object_tracker_intensity();
}

static void mdlProcessParameters_c1_object_tracker_intensity(SimStruct *S)
{
  int i;
...
static void mdlSetWorkWidths_c1_object_tracker_intensity(SimStruct *S)
{
  if(sim_mode_is_rtw_gen(S)) {
    int_T chartIsInlinable =
      (int_T)sf_is_chart_inlinable("object_tracker_intensity",1);
    ssSetStateflowIsInlinable(S,chartIsInlinable);
    ssSetEnableFcnIsTrivial(S,1);
    ssSetDisableFcnIsTrivial(S,1);
    ssSetNotMultipleInlinable(S,sf_rtw_info_uint_prop("object_tracker_intensity",1,"gatewayCannotBeInlinedMultipleTimes"));
    if(chartIsInlinable) {
      ssSetInputPortOptimOpts(S, 0, SS_REUSABLE_AND_LOCAL);
      ssSetInputPortOptimOpts(S, 1, SS_REUSABLE_AND_LOCAL);
      sf_mark_chart_expressionable_inputs(S,"object_tracker_intensity",1,2);
    }
    if (!sf_is_chart_instance_optimized_out("object_tracker_intensity",1)) {
      int dtId;
      char *chartInstanceTypedefName =
        sf_chart_instance_typedef_name("object_tracker_intensity",1);
...
static void mdlRTW_c1_object_tracker_intensity(SimStruct *S)
{
  if(sim_mode_is_rtw_gen(S)) {
    sf_write_symbol_mapping(S, "object_tracker_intensity", 1);
    ssWriteRTWStrParam(S, "StateflowChartType", "Embedded MATLAB");
  }
}

static void
sf_save_debug_c1_object_tracker_intensity(SFc1_object_tracker_intensityInstanceStruct*
...
static void
sf_load_debug_c1_object_tracker_intensity(SFc1_object_tracker_intensityInstanceStruct*
 chartInstance,FILE* file)
{
  int machineNumber = _object_tracker_intensityMachineNumber_;
  int chartNumber = chartInstance->chartNumber;
...
static void sf_c1_object_tracker_intensity_sim_ctx_io(SimStruct* S, const char
 io, FILE* file)
{
  if(io == 'r') {
    sf_load_state_c1_object_tracker_intensity(file);
    sf_load_debug_c1_object_tracker_intensity(&chartInstance, file);
  } else {
    sf_save_state_c1_object_tracker_intensity(file);
    sf_save_debug_c1_object_tracker_intensity(&chartInstance, file);
  }
}
static void mdlStart_c1_object_tracker_intensity(SimStruct *S)
{
  chartInstance.chartInfo.chartInstance = NULL;
  chartInstance.chartInfo.isEMLChart = 1;
  chartInstance.chartInfo.chartInitialized = 0;
  chartInstance.chartInfo.sFunctionGateway =
    sf_opaque_gateway_c1_object_tracker_intensity;
  chartInstance.chartInfo.initializeChart =
    sf_opaque_initialize_c1_object_tracker_intensity;
  chartInstance.chartInfo.terminateChart =
    sf_opaque_terminate_c1_object_tracker_intensity;
  chartInstance.chartInfo.enableChart =
    sf_opaque_enable_c1_object_tracker_intensity;
  chartInstance.chartInfo.disableChart =
    sf_opaque_disable_c1_object_tracker_intensity;
  chartInstance.chartInfo.mdlRTW = mdlRTW_c1_object_tracker_intensity;
  chartInstance.chartInfo.mdlStart = mdlStart_c1_object_tracker_intensity;
  chartInstance.chartInfo.mdlSetWorkWidths =
    mdlSetWorkWidths_c1_object_tracker_intensity;
  chartInstance.chartInfo.restoreLastMajorStepConfiguration = NULL;
  chartInstance.chartInfo.restoreBeforeLastMajorStepConfiguration = NULL;
  chartInstance.chartInfo.storeCurrentConfiguration = NULL;
  chartInstance.S = S;
  ssSetUserData(S,(void *)(&(chartInstance.chartInfo))); /* register the chart instance with simstruct */
  ssSetmdlSimulationContextIO(S, sf_c1_object_tracker_intensity_sim_ctx_io);
  if(!sim_mode_is_rtw_gen(S)) {
    init_dsm_address_info();
  }
}

void c1_object_tracker_intensity_method_dispatcher(SimStruct *S, int_T method,
 void *data)
{
  switch (method) {
   case SS_CALL_MDL_START:
    mdlStart_c1_object_tracker_intensity(S);
    break;
   case SS_CALL_MDL_SET_WORK_WIDTHS:
    mdlSetWorkWidths_c1_object_tracker_intensity(S);
    break;
   case SS_CALL_MDL_PROCESS_PARAMETERS:
    mdlProcessParameters_c1_object_tracker_intensity(S);
    break;
   default:
    /* Unhandled method */
    sf_mex_error_message("Stateflow Internal Error:\n"
     "Error calling c1_object_tracker_intensity_method_dispatcher.\n"
